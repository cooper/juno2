...
sub start {
  ...
  $user->sendnum('005','CHANTYPES=# EXCEPTS INVEX CHANMODES=,,,mnt PREFIX=(qaohv)~&@%+ NETWORK='.::conf('server','network').' STATUSMSG=@+ MODES='.::conf('main','maxmodes').' :are support by this server');
  $user->handle_lusers;
  $user->handle_motd;
